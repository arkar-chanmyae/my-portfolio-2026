                ...
                >
                  <div className="md:max-w-[60%]">
                    <div className="flex items-start justify-between mb-6 gap-2 flex-wrap">
                      {(project.categories || []).map((cat: string) => <Tag key={cat}>{cat}</Tag>)}
                    </div>
                    <h3 className="text-xl font-light mb-3">{project.title}</h3>
                    <p className="text-sm text-black/45 leading-relaxed mb-8">{project.description}</p>
                  </div>
                  <div className="flex gap-8 pt-6 border-t border-black/[0.06]">
                    {project.liveUrl && (
                      <div>
                        <a href={project.liveUrl} target="_blank" rel="noreferrer" className="text-sm font-light hover:underline">Live Project ↗</a>
                      </div>
                    )}
                    {project.repositoryUrl && (
                      <div>
                        <a href={project.repositoryUrl} target="_blank" rel="noreferrer" className="text-sm font-light hover:underline">Source Code ↗</a>
                      </div>
                    )}
                  </div>
                </div>
