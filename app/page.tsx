...
export default function AgenticPage() {
  ...
  return (
    <div className="bg-[#F5F4F0] text-[#111] min-h-screen font-sans antialiased">
      ...
      <section className="relative h-screen overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 z-30 flex flex-col px-6 md:px-12 pb-12 max-w-3xl">
          ...
          >
            {portfolio?.hero ? (
              <>
                {portfolio.hero.fullName}<br />
                <span className="text-3xl sm:text-4xl text-black/60 block mt-4">{portfolio.hero.tagline}</span>
              </>
            ) : (
              "Loading..."
            )}
          ...
          >
            ...
            {portfolio?.hero?.resumeFile ? (
              ...
              <button
                onClick={() => alert("CV file is not configured in the database yet. Convert your Google Drive link to direct download and set it in hero.resumeFile!")}
                className="px-8 py-3.5 bg-white/60 border border-black/10 text-[#111] text-xs rounded-xl hover:bg-black/5 hover:border-black/20 transition-all tracking-widest font-semibold font-sans flex items-center justify-center min-w-[140px]"
                style={{ backdropFilter: "blur(8px)" }}
              >
                DOWNLOAD CV
      ...
      <section id="education" className="py-32 px-6 md:px-12 lg:px-20">
          ...
          <div className="grid grid-cols-12 grid-rows-auto gap-3" onMouseMove={handleMouse}>
            ...
            <BentoCard className="col-span-12 p-8 min-h-[200px] flex flex-col justify-between relative overflow-hidden" delay={0}>
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-xl font-light mb-1">{portfolio?.education?.[0]?.degree || "Loading..."}</h3>
                <div className="text-sm font-medium mb-3">{portfolio?.education?.[0]?.institution}</div>
                ...
                <div className="mt-4 text-xs text-black/35 font-mono">
                  {portfolio?.education?.[0]?.startDate} - {portfolio?.education?.[0]?.endDate}
                </div>
              </div>
            </BentoCard>

            {/* Bottom row */}
            <BentoCard className="col-span-12 md:col-span-4 p-8 min-h-[200px] flex flex-col justify-between" delay={120}>
              <div>
                <div className="w-10 h-10 rounded-xl border border-black/10 flex items-center justify-center mb-5">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
                </div>
                <h3 className="text-lg font-light mb-1">{portfolio?.education?.[1]?.degree || "Loading..."}</h3>
                <div className="text-xs font-medium mb-2">{portfolio?.education?.[1]?.institution}</div>
                <p className="text-sm text-black/45 leading-relaxed">{portfolio?.education?.[1]?.description || ""}</p>
              </div>
              <div className="mt-4 text-xs text-black/35 font-mono">
                {portfolio?.education?.[1]?.startDate} - {portfolio?.education?.[1]?.endDate}
              </div>
            </BentoCard>

            <BentoCard className="col-span-12 md:col-span-4 p-8 min-h-[200px] flex flex-col justify-between" delay={160}>
              <div>
                <div className="w-10 h-10 rounded-xl border border-black/10 flex items-center justify-center mb-5">
                ...
                <h3 className="text-lg font-light mb-1">{portfolio?.education?.[2]?.degree || "Loading..."}</h3>
                <div className="text-xs font-medium mb-2">{portfolio?.education?.[2]?.institution}</div>
                <p className="text-sm text-black/45 leading-relaxed">{portfolio?.education?.[2]?.description || ""}</p>
              </div>
              <div className="mt-4 text-xs text-black/35 font-mono">
                {portfolio?.education?.[2]?.startDate} - {portfolio?.education?.[2]?.endDate}
              </div>
            </BentoCard>

            <BentoCard className="col-span-12 md:col-span-4 p-8 min-h-[200px] flex flex-col justify-between" delay={200}>
              <div>
                <div className="w-10 h-10 rounded-xl border border-black/10 flex items-center justify-center mb-5">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                </div>
                <h3 className="text-lg font-light mb-1">{portfolio?.education?.[3]?.degree || "Loading..."}</h3>
                <div className="text-xs font-medium mb-2">{portfolio?.education?.[3]?.institution}</div>
                <p className="text-sm text-black/45 leading-relaxed">{portfolio?.education?.[3]?.description || ""}</p>
              </div>
              <div className="mt-4 text-xs text-black/35 font-mono">
                {portfolio?.education?.[3]?.startDate} - {portfolio?.education?.[3]?.endDate}
              </div>
            </BentoCard>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8 mb-16">
            ...
            <p className="text-sm text-black/45 leading-relaxed max-w-xs md:self-end">
              Here is a selection of my latest work, spanning web development, tools, and systems design.
            </p>
          </div>

          <StackingAgentCards projects={portfolio?.projects || []} />
        </div>
      </section>

      {/* ── PROFESSIONAL SKILLS ──────────────────────────────────────────── */}
      <section id="skills" className="py-32 px-6 md:px-12 lg:px-20 border-t border-black/[0.06] overflow-hidden">
        <div className="max-w-6xl mx-auto">
          <div className="mb-16">
            <div className="flex items-center gap-3 mb-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3" onMouseMove={handleMouse}>
            {portfolio?.skills ? portfolio.skills.slice(0, 4).map((skill: any, index: number) => (
              <BentoCard key={skill.id || index} className="relative overflow-hidden flex flex-col min-h-[320px]" delay={index * 60}>
                {/* Number top-left */}
                ...
                <div className="relative z-10 px-7 pb-7 mt-auto pt-4">
                  ...
                  <p className="text-sm text-black/45 leading-relaxed">{skill.description}</p>
                </div>
              </BentoCard>
            )) : (
              <div>Loading...</div>
      ...
      {/* ── INTEGRATIONS ──────────────────────────────────────────────────── */}
      {/* ── TECH STACK ────────────────────────────────────────────────────── */}
      <section id="techstack" className="py-32 px-6 md:px-12 lg:px-20 border-t border-black/[0.06]">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8 mb-16">
            <div>
              <div className="flex items-center gap-3 mb-3">
            ...
            <p className="text-sm text-black/45 leading-relaxed max-w-xs md:self-end">
              A comprehensive list of technologies, frameworks, and tools I have experience working with.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {portfolio?.techStack ? portfolio.techStack.map((stack: any, index: number) => (
              <BentoCard key={stack.id || index} className="p-8 flex flex-col min-h-[240px]" delay={index * 80}>
                <Tag>{stack.category}</Tag>
                <div className="mt-6 flex flex-wrap gap-2">
                  {(stack.items || []).map((item: string, i: number) => (
                    <span key={i} className="inline-flex items-center px-3 py-1.5 rounded-lg border border-black/10 bg-white text-sm text-black/70 shadow-sm">
                      {item}
                    </span>
                  ))}
                </div>
      ...
      <section id="contact" className="relative py-32 px-6 md:px-12 lg:px-20 border-t border-black/[0.06] overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mx-auto text-center">
          ...
          <p className="text-sm text-black/45 leading-relaxed mb-10">
            {portfolio?.contact?.tagline || "Get in touch to discuss opportunities."}
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            {portfolio?.contact?.email && (
              <a href={`mailto:${portfolio.contact.email}`} className="px-8 py-3 bg-[#111] text-white text-sm rounded-xl hover:bg-[#333] transition-colors tracking-widest font-medium">
                EMAIL ME
              </a>
            )}
            {portfolio?.contact?.phone && (
              <a href={`tel:${portfolio.contact.phone}`} className="px-8 py-3 bg-white border border-black/10 text-[#111] text-sm rounded-xl hover:bg-black/5 transition-colors tracking-widest font-medium">
                CALL ME
              </a>
            )}
          </div>
      ...
      <footer className="py-10 px-6 md:px-12 lg:px-20 border-t border-black/[0.06]">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          <span className="font-pixel text-xs tracking-[0.25em] text-black/50 uppercase">{portfolio?.hero?.fullName || "PORTFOLIO"}</span>
          ...
          <div className="flex flex-wrap items-center gap-x-8 gap-y-3">
            {[
              ...
              { label: "Experience",   href: "#workflow" },
              { label: "Tech Stack",   href: "#integrations" },
            ].map(l => (
              <a key={l.label} href={l.href} className="text-xs text-black/35 hover:text-black/70 transition-colors tracking-widest uppercase">{l.label}</a>
            ))}
          </div>
          ...
          <div className="flex items-center gap-6">
            {(portfolio?.socialLinks || []).map((link: any) => (
              <a key={link.platform} href={link.url} target="_blank" rel="noreferrer" className="text-xs text-black/25 hover:text-black/55 transition-colors tracking-widest uppercase">{link.platform}</a>
            ))}
          </div>
        </div>
        <div className="max-w-6xl mx-auto mt-8 pt-6 border-t border-black/[0.04]">
          <span className="text-xs text-black/20">© {new Date().getFullYear()} {portfolio?.hero?.fullName}. All rights reserved.</span>
        </div>
      </footer>
    </div>
  )
